...
namespace solutions
{
	SolutionTwoPartTwo::SolutionTwoPartTwo() : m_totalPower(0)
	{
		work();
	}

	std::string SolutionTwoPartTwo::answer()
	{
		return m_answer;
	}
	/**
	...
	*/
	void SolutionTwoPartTwo::work()
	{
		//open file
		std::ifstream solutionTwoFile;
		solutionTwoFile.open("SolutionTwo.txt");
		//create iterator for game number
		//import first line
		std::string tempLine;
		if (solutionTwoFile.is_open())
		{
			while (getline(solutionTwoFile, tempLine))
			{
				//iterate game number
				int largestRedNum = 0;
				int largestBlueNum = 0;
				int largestGreenNum = 0;

				//check all reds
				for (size_t i = tempLine.find("red"); i != -1; i = tempLine.find("red", i + 1))
				{
					std::string tempNumber = tempLine.substr(i - 3, 3);
					tempNumber.erase(remove(tempNumber.begin(), tempNumber.end(), ' '), tempNumber.end());
					if (stoi(tempNumber) > largestRedNum)
					{
						largestRedNum = stoi(tempNumber);
					}
				}
				//check all blues
				for (size_t i = tempLine.find("blue"); i != -1; i = tempLine.find("blue", i + 1))
				{
					std::string tempNumber = tempLine.substr(i - 3, 3);
					tempNumber.erase(remove(tempNumber.begin(), tempNumber.end(), ' '), tempNumber.end());
					if (stoi(tempNumber) > largestBlueNum)
					{
						largestBlueNum = stoi(tempNumber);
					}
				}
				//check all greens
				for (size_t i = tempLine.find("green"); i != -1; i = tempLine.find("green", i + 1))
				{
					std::string tempNumber = tempLine.substr(i - 3, 3);
					tempNumber.erase(remove(tempNumber.begin(), tempNumber.end(), ' '), tempNumber.end());
					if (stoi(tempNumber) > largestGreenNum)
					{
						largestGreenNum = stoi(tempNumber);
					}
				}
				m_totalPower += (largestRedNum * largestBlueNum * largestGreenNum);
				

			}
			m_answer = std::to_string(m_totalPower);
		}
		else
		{
			m_answer = "Error opening file.";
		}
		solutionTwoFile.close();
	}
} // namespace solutions
